...
async function compareP2Current(p2Current, p1Current, c2Current) {
    ...
    if (difference <= allowableDifference) {
        // All OK within the margin
        p2StatusElement.innerHTML = `<span class="ok">All OK in P2 Region</span>`;
        return; // No further action needed
    }

    // Mismatch detected, start theft detection process
    p2StatusElement.innerHTML = `<span class="not-ok">Mismatch detected</span>`;
    console.log("Mismatch detected. Initiating theft detection process...");

    // Step 1: Set LED of C2 to false for 15 seconds
    await update(c2Ref, { LED: false });
    console.log("LED of C2 set to false for 15 seconds");

    setTimeout(async () => {
        // Step 2: Fetch real-time P1 and P2 values
        ...
        // Step 3: Recheck assuming C2 = 0
        const expectedCurrentWithZeroC2 = p1RealTimeCurrent; // Assume C2 = 0
        const differenceWithZeroC2 = Math.abs(p2RealTimeCurrent - expectedCurrentWithZeroC2);

        if (differenceWithZeroC2 > tolerance) {
            // Theft confirmed
            p2StatusElement.innerHTML = `<span class="not-ok">Theft confirmed between P1 and P2</span>`;
            console.log("Theft confirmed between P1 and P2");

            // Set LED of C2 back to true for further monitoring
            await update(c2Ref, { LED: true });
            console.log("LED of C2 reset to true after confirming theft");

            // Wait 1 minute for consumer to resolve issue
            setTimeout(async () => {
                console.log("Rechecking after 1 minute...");

                // Fetch real-time P1, P2, and C2 values
                const p1Snapshot = await get(ref(db, 'P1'));
                const p2Snapshot = await get(ref(db, 'P2'));
                ...
                if (newDifference > allowableDifference) {
                    // Theft still detected
                    p2StatusElement.innerHTML = `<span class="not-ok">Consumer theft confirmed. Disconnecting...</span>`;
                    console.log("Consumer theft confirmed. Disconnecting...");

                    // Permanently set LED to false
                    await update(c2Ref, { LED: false });
                    console.log("LED of C2 permanently set to false due to repeated theft");
                } else {
                    // No theft detected, consumer fault corrected
                    p2StatusElement.innerHTML = `<span class="ok">Consumer corrected fault</span>`;
                    console.log("Consumer corrected fault");

                    // Restart the process to monitor
